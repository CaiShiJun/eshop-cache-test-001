...
import org.github.caishijun.eshop.storm.http.HttpClientUtils;
import org.github.caishijun.eshop.storm.zk.ZooKeeperSession;
                    ...
                    for(Map.Entry<Long, Long> productCountEntry : productCountList) {
                        if(productCountEntry.getValue() > 10 * avgCount) {
                            ...
                            if(!lastTimeHotProductIdList.contains(productCountEntry.getKey())) {
                                ...
                                String distributeNginxURL = "http://192.168.74.103/hot?productId=" + productCountEntry.getKey();
                                HttpClientUtils.sendGetRequest(distributeNginxURL);

                                // 将缓存热点，那个商品对应的完整的缓存数据，发送请求到缓存服务去获取，反向推送到所有的后端应用nginx服务器上去
                                String cacheServiceURL = "http://192.168.74.1:8080/getProductInfo?productId=" + productCountEntry.getKey();
                                ...
                                String[] appNginxURLs = new String[]{
                                        "http://192.168.74.101/hot?productId=" + productCountEntry.getKey() + "&" + productInfo,
                                        "http://192.168.74.102/hot?productId=" + productCountEntry.getKey() + "&" + productInfo
                                };
                    ...
                    if(lastTimeHotProductIdList.size() == 0) {
                        ...
                        for(Long productId : lastTimeHotProductIdList) {
                            if(!hotProductIdList.contains(productId)) {
                                ...
                                String url = "http://192.168.74.103/cancel_hot?productId=" + productId;
